  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (data.messages && data.messages[0]) {
        ...
        setTotalTokens({
          prompt:
            data.tokenUsage?.promptTokens || data.tokenUsage?.input_tokens || 0,
          completion:
            data.tokenUsage?.completionTokens ||
            data.tokenUsage?.output_tokens ||
            0,
          total:
            data.tokenUsage?.totalTokens || data.tokenUsage?.total_tokens || 0,
        });
      }
    } catch (error) {
      ...
      <header className='border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-50 flex-none'>
        <div className='container mx-auto px-4 py-4 flex items-center justify-between'>
          <div className='flex items-center space-x-4'>
            <div
              className={`w-10 h-10 ${currentModel.color} rounded-lg flex items-center justify-center`}
            >
